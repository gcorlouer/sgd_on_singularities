...
class SGDPolyRunner:
    def __init__(self, nSGD=10**4, nsamples=10**3, batch_size=30, lr=0.01, 
        ...
        """
        nSGD: number of SGD runs
        nsamples: number of data points (number of timesteps = nsamples/batch_size)
        """
        self.nSGD = nSGD
        ...
        for xb, yb in dataset:
            ...
            if self.auto==True:
                ...
                for param in model.parameters():
                    print("grad 0", param.grad)
                loss.backward()
                for param in model.parameters():
                    print("grad 1", param.grad)
                optimizer.step()
            else:
    ...
    def parameter_sweep(self, w0_range, batch_range, lr_range,
        ...
        fpath = Path("../data_guillaume/regressions")
        fpath = fpath.joinpath(fname)
        df.to_csv(fpath)
        return df
